...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        if (pid == 0)
        {
            ...
            //access.logというファイルを開く
            fp = fopen("access.log", "a");
            if (fp == NULL)
            {
                perror("access.logがありません");
            }
            else
            {
                char day[25];
                ...
                strftime(day, 25, "%d/%b/%Y:%T %Z", timeptr);

                //access.logにアクセスログを保存する
                fprintf(fp, "%s - - [%s] \"%s %s %s\" %d %d\n", inet_ntoa(ca.sin_addr), day, meth, ind, ver, httpstate, buf_size);
            }
            //ファイルを閉じる
            fclose(fp);
